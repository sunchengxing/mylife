const HousingPage = {
  ...
  showModal(item) {
    ...
    overlay.querySelector("#f-save").onclick = async () => {
      ...
      await dbPut("housing", { id: item?.id||genId(), name, category: document.getElementById("f-category").value.trim(), cost: document.getElementById("f-cost").value||"", note: document.getElementById("f-note").value.trim(), created_at: item?.created_at||new Date().toISOString(), updated_at: new Date().toISOString() });
