const TransportPage = {
  async render() {
    ...
    container.innerHTML = SearchFilter.renderBar("transport") +
      SearchFilter.renderSummary(items, "transport") +
      this._renderCards(items);
    setLang(currentLang);
    SearchFilter.bindEvents(TransportPage, "transport");
  },
  ...
  showModal(item) {
    ...
    overlay.querySelector("#f-save").onclick = async () => {
      ...
      await dbPut("transport", { id: item?.id||genId(), name, category: document.getElementById("f-category").value.trim(), cost: document.getElementById("f-cost").value||"", note: document.getElementById("f-note").value.trim(), photo: getPhotoFromPreview(), created_at: item?.created_at||new Date().toISOString(), updated_at: new Date().toISOString() });
